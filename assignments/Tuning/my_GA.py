    ...
    def evaluate(self, decision):
        ...
        if decision not in self.evaluated:
            ...
            for fold in range(self.crossval_fold):
                ...
                train_indices = indices[:start] + indices[end+1:]
            ...
            objs_crossval = np.array(objs_crossval).mean(axis=0)
        ...
        for i, x in enumerate(self.generation):
            ...
            for j in range(len(x)):
                if np.random.random() < self.mutation_rate:
                    ...
                    if type(boundary) == list:
                        val = boundary[np.random.choice(len(boundary))]
                        if type(boundary[0]) == int:
                            val = round(val)
                        new_x[j] = val
                    else:
                        new_x[j] = boundary[np.random.choice(len(boundary))]
                        if type(new_x[j]) == int:
